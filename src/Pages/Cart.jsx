...
const Cart = ({ cartItems, removeFromCart, addToCart }) => {
  if (!cartItems.length) return(
     <div className=' w-[100vw] h-[100vh] flex items-center justify-center flex-col '>
       ...
       <Link to='/'> <button className="bg-[#343A40]  text-white text-sm py-2 px-3 rounded flex flex-row items-center gap-2" >
    ...
    <div className='flex flex-col pt-7 w-[100vw] gap-5'>
     <h2 className='font-semibold text-3xl'>Cart</h2>
          ...
          <li key={item.id} className='flex w-[95vw] border-b-2 flex-row items-center justify-between p-3 pb-7'>
           ...
           {item.name}
            <div className='flex flex-row items-center justify-between gap-1'>
              <button onClick={() => handleRemoveOne(item)}><img src= {minus}/></button>
              <span>{item.quantity}</span>
              <button onClick={() => handleAddOne(item)}><img src={plus} alt="" /></button>
            </div>
                       <span className='font-semibold text-xl'>£{item.price.toFixed(2)}</span>
      ...
      <h2 className='border-b-2'>Order Summary</h2> 
        ...
        <h3 className='flex flex-row justify-between font-semibold text-lg text-[#343A40]'>Subtotal: <span>£{(totalAmount+10).toFixed(2)}</span></h3>
